  ...
  return <>
    ...
    <main className={styles.main}>
      ...
      <section className={`${styles.card} ${styles.vision}`}>
        <div className={styles.number}>12</div><div><span className="eyebrow">BEFORE THE SEARCH</span><h2>Local screenshot recognition</h2><p>The vision worker locates the repeated board grid, normalizes its geometry, segments tile colors, extracts large letter glyphs separately from their point values, matches them against compact templates, and detects rack slots and physical blanks.</p><Formula block tex={'\\text{Screenshot} \\longrightarrow \\text{Recognized Board + Rack} \\longrightarrow \\text{Deterministic Solver}'}/><p>Confidence values mark readings worth reviewing. Every tile remains editable, and importing over manual state produces explicit conflicts instead of silently overwriting prior work. Recognition and solving are independent browser workers; screenshot bytes stay on the device and no remote AI API is required.</p></div>
      </section>
