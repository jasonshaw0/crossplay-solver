...
export default function Solver() {
  ...
  const [showSettings,setShowSettings]=useState(false),[showMobileResults,setShowMobileResults]=useState(false),[showMobileScore,setShowMobileScore]=useState(false),[draft,setDraft]=useState(defaultSettings),[corrections,setCorrections]=useState<CorrectionSample[]>([]);
  const worker=useRef<Worker|null>(null),revision=useRef(0),latestPosition=useRef(position),pendingAutoSolve=useRef<Position|null>(null),boardSection=useRef<HTMLElement>(null),screenshotInput=useRef<HTMLInputElement>(null),jsonInput=useRef<HTMLInputElement>(null),dictInput=useRef<HTMLInputElement>(null);
  ...
  useEffect(()=>{
    ...
    current.onmessage=event=>{
      ...
      if(message.type==='solved'){setMoves(message.moves);setStats(message.stats);setStatus(`${message.moves.length.toLocaleString()} legal plays found.`);setShowMobileScore(false);if(window.matchMedia('(max-width:740px)').matches)setShowMobileResults(true);}
    };
  ...
  function commit(next:Position,message='') {
    ...
    revision.current++;setMoves(null);setPreview(null);setStats(null);setIssues([]);setError('');setStatus(message);setSolving(false);setShowMobileResults(false);setShowMobileScore(false);pendingAutoSolve.current=null;
  }
  ...
  function solve(target=latestPosition.current,progress='') {
    ...
    setSolving(true);setError('');setStatus('');setMoves(null);setPreview(null);setStats(null);setShowMobileScore(false);
  ...
  async function importScreenshot(file:File) {
    ...
    try {
      ...
      const reviewCount=Object.keys(reconciled.uncertain).length;
      commit(reconciled.position,`Screenshot imported · ${result.board.tiles.length} board tiles · ${result.rack.length} rack tiles · ${Math.round(result.timings.totalMs)} ms${reviewCount?` · Review ${reviewCount} marked reading${reviewCount===1?'':'s'}`:''}`);
      ...
      const canAutoSolve=result.board.detected&&result.board.complete&&result.board.tiles.length>0&&result.rack.length===7&&reconciled.position.rack.every(Boolean)&&reconciled.conflicts.length===0&&Object.keys(reconciled.uncertain).length===0;
  ...
  function applyMove(move:Move) {
    ...
    setShowMobileResults(false);setShowMobileScore(false);commit({...position,board,rack,manualCells:{}},`${move.mainWord} applied. Refill your rack for the next solve.`);
  }
  function showPreviewOnBoard() {
    setShowMobileResults(false);setShowMobileScore(false);
    requestAnimationFrame(()=>boardSection.current?.scrollIntoView({behavior:'smooth',block:'center'}));
  }
  ...
  const problemCells=useMemo(()=>new Set(issues.filter(issue=>issue.severity==='error').flatMap(issue=>issue.cells.map(p=>`${p.row},${p.col}`))),[issues]),hasUncertain=Object.values(uncertain).some(Boolean);
  const selectedConflict=conflicts.find(c=>c.row===selected.row&&c.col===selected.col);
  return <>
    <header className="app-header"><SiteBrand/><div className="header-actions"><span className="local-badge"><i/>Runs on your device</span><a className="button subtle methodology-link" href={`${BASE_PATH}/methodology/`}>Methodology</a><button className="button subtle settings-trigger" aria-label="Settings" onClick={()=>{setDraft(settings);setShowSettings(!showSettings);}} aria-expanded={showSettings} aria-haspopup="dialog"><Icon name="settings"/><span>Settings</span></button></div></header>
    <main className="workspace">
      <div className={`solver-layout ${occupied===0?'empty-position':''}`}>
        {occupied===0&&screenshotCard('mobile-import-first')}
        <section ref={boardSection} className="board-section" aria-labelledby="board-title"><div className="section-heading board-heading"><div><span className="eyebrow">YOUR POSITION</span><h2 id="board-title">The board <span className="small-count">{occupied} tiles</span></h2></div><div className="direction-switch" aria-label="Typing direction"><button className={direction==='across'?'active':''} aria-pressed={direction==='across'} onClick={()=>setDirection('across')}>Across →</button><button className={direction==='down'?'active':''} aria-pressed={direction==='down'} onClick={()=>setDirection('down')}>Down ↓</button></div></div>
          ...
          {(preview||hasUncertain)&&<div className="board-state-key">{preview&&<span><i className="key-new"/>Blue tiles are new · outlined tiles complete scored words</span>}{hasUncertain&&<span><i className="key-review">?</i>Review this screenshot reading</span>}</div>}
      ...
      {(recognition?.warnings.length||conflicts.length>0||issues.length>0)?<details className="review-panel" open={hasUncertain||conflicts.length>0||issues.some(i=>i.severity==='error')}><summary>Review position · {conflicts.length} conflict{conflicts.length!==1?'s':''}{hasUncertain?' · marked cells need a look':''}</summary>{recognition?.warnings.map((warning,i)=><p key={i}>{warning}</p>)}{issues.map((issue,i)=><p key={i}>{issue.message}</p>)}{conflicts.map((conflict,i)=><div key={i}><span>{conflict.message}</span><button onClick={()=>acceptConflict(conflict)}>Use screenshot reading</button></div>)}</details>:null}
      ...
      <Modal open={showMobileResults} onClose={()=>setShowMobileResults(false)} labelledBy="mobile-results-title" panelClassName="mobile-results-panel" backdropClassName="mobile-results-backdrop"><div className="mobile-results-heading"><div><span className="eyebrow">FIND YOUR NEXT PLAY</span><h2 id="mobile-results-title">{showMobileScore?'Move details':'Best moves'} {moves&&<span className="count">{moves.length.toLocaleString()}</span>}</h2>{stats&&!showMobileScore&&<p>{Math.round(stats.durationMs)} ms · exact scores</p>}</div><button className="icon-button modal-close" data-modal-close aria-label="Close results" onClick={()=>setShowMobileResults(false)}><Icon name="close"/></button></div>{showMobileScore&&preview?<><button className="text-button mobile-results-back" onClick={()=>setShowMobileScore(false)}>← All moves</button><ScoreBreakdown move={preview} onClose={()=>setShowMobileScore(false)} onApply={()=>applyMove(preview)}/></>:<><Results moves={moves} stats={stats} selected={preview} onSelect={move=>{setPreview(move);setShowMobileScore(false);}} solving={solving} titleId="mobile-results-title" hideHeading/>{preview&&<div className="mobile-selected-move"><div><span>SELECTED MOVE</span><strong>{preview.mainWord}</strong><small>{preview.totalScore} points · {String.fromCharCode(65+preview.startCol)}{preview.startRow+1} {preview.direction==='across'?'→':'↓'}</small></div><div className="mobile-selected-actions"><button className="button" onClick={showPreviewOnBoard}>Show on board</button><button className="button primary" onClick={()=>setShowMobileScore(true)}>Details</button></div></div>}</>}</Modal>
      {moves&&!showMobileResults&&<button className={`button primary mobile-results-reopen ${preview?'has-selection':''}`} onClick={()=>{setShowMobileScore(false);setShowMobileResults(true);}}><Icon name={preview?'check':'spark'}/>{preview?<><strong>{preview.mainWord} selected</strong><span>{preview.totalScore} pts · Change or apply</span></>:<>View best moves <span>{moves.length.toLocaleString()}</span></>}</button>}
